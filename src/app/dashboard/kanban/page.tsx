...
function KanbanColumn({ status, leads }: { status: LeadStatus; leads: Lead[] }) {
  ...
  // extract bg color class for the dot indicator
  const dotColor = cfg.color.split(" ")[0];

  return (
    <div className="flex w-64 shrink-0 flex-col rounded-xl border border-gray-200 bg-gray-50 max-h-[calc(100vh-180px)]">
      {/* Header */}
      <div className="flex items-center justify-between rounded-t-xl border-b border-gray-200 bg-white px-4 py-3 shrink-0">
      ...
      >
        {leads.length === 0 && (
          <div className={`my-2 rounded-lg border-2 border-dashed py-8 text-center text-xs transition-colors ${
            isOver ? "border-primary/40 text-primary" : "border-gray-200 text-gray-400"
          }`}>
...
export default function KanbanPage() {
  ...
  return (
    <div className="flex flex-col h-full">
      <h1 className="mb-6 text-2xl font-bold text-gray-900 shrink-0">Kanban</h1>
      <DndContext
        sensors={sensors}
