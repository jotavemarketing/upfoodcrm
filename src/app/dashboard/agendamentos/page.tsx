...
const STATUS_CONFIG: Record<AgendamentoStatus, { label: string; color: string }> = {
  pendente: { label: "Pendente", color: "bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-400" },
  realizado: { label: "Realizado", color: "bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400" },
  cancelado: { label: "Cancelado", color: "bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400" },
};

type AgendamentoWithLead = Agendamento & { leads: { name: string; company: string } | null };
...
export default function AgendamentosPage() {
  ...
  async function load() {
    const { data } = await supabase
      .from("agendamentos").select("*, leads(name, company)").order("data_hora", { ascending: true });
  ...
  const filtered = filterStatus === "todos" ? agendamentos : agendamentos.filter((a) => a.status === filterStatus);
  const pendentes = agendamentos.filter((a) => a.status === "pendente").length;

  if (loading) {
    return <div className="flex h-64 items-center justify-center"><p className="text-sm text-gray-400">Carregando...</p></div>;
  }
    ...
    <div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((ag) => (
            <div key={ag.id} className="flex items-start gap-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <p className="text-sm font-semibold text-gray-900 dark:text-white">{ag.titulo}</p>
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_CONFIG[ag.status].color}`}>
                    {STATUS_CONFIG[ag.status].label}
                  </span>
                </div>
                <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">{formatDate(ag.data_hora)}</p>
                {ag.leads && (
                  <button
                    onClick={() => router.push(`/dashboard/leads/${ag.lead_id}`)}
                    className="mt-1 flex items-center gap-1 text-xs text-gray-400 hover:text-primary dark:hover:text-red-400 transition-colors"
                  >
                ...
                {ag.descricao && <p className="mt-1.5 text-xs text-gray-400 dark:text-gray-500">{ag.descricao}</p>}
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <select
                  value={ag.status}
                  onChange={(e) => handleStatusChange(ag.id, e.target.value as AgendamentoStatus)}
                  className="rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2 py-1.5 text-xs outline-none"
                >
                ...
                <button
                  ...
                  className="rounded-lg p-1.5 text-gray-300 dark:text-gray-600 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors"
                >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 shadow-xl">
            ...
            <div className="space-y-4">
              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Lead *</label>
              ...
              <div>
                ...
                <input value={form.titulo} onChange={(e) => setForm((p) => ({ ...p, titulo: e.target.value }))} placeholder="Ex: Reunião de proposta" className={inputCls} />
              </div>
              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Data e hora *</label>
                <input type="datetime-local" value={form.data_hora} onChange={(e) => setForm((p) => ({ ...p, data_hora: e.target.value }))} className={inputCls} />
              </div>
              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Descrição</label>
                <textarea value={form.descricao} onChange={(e) => setForm((p) => ({ ...p, descricao: e.target.value }))} rows={3} placeholder="Detalhes..." className={inputCls} />
              </div>
            </div>
            <div className="mt-4 flex gap-3">
              <button onClick={() => setShowModal(false)} className="flex-1 rounded-lg border border-gray-300 dark:border-gray-600 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">
                Cancelar
              </button>
              <button onClick={handleCreate} disabled={saving || !form.titulo || !form.data_hora || !form.lead_id} className="flex-1 rounded-lg bg-primary py-2.5 text-sm font-semibold text-white hover:bg-primary-dark disabled:opacity-60">
